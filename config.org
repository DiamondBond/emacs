...
**** Description
This is only useful when using DejaVu, Quivira or Symbola fonts.
**** Code
#+begin_src emacs-lisp
...
** Config
*** =config/tangle=
Executes [[https://github.com/DiamondBond/emacs/blob/master/tangle.sh][tangle.sh]].
#+begin_src emacs-lisp
  (defun config/tangle ()
	"Tangle this Emacs configuration."
	(interactive)
	(when (file-readable-p
		   (concat user-emacs-directory "tangle.sh"))
	  (shell-command "tangle.sh")))
#+end_src
*** =config/reload=
