...
#+begin_src sh
  ...
  mkdir -p ~/.emacs.d
  cp -r ~/git/emacs/{early-init.el,init.el,config.org,snippets,img} ~/.emacs.d/
...
* Dependencies
** Packages
This section is just a raw dump of packages that I have needed in the past to comfortably run this Emacs distribution under Ubuntu 20.04, not everything here is required, this is just a rough guideline.
