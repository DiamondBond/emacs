...
This literate configuration along with its auxiliary files, folders & dependencies (specified [[https://github.com/DiamondBond/emacs/blob/master/docs/dependencies.org][here]]) should result in my opinionated Emacs HCI.
...
- Modular completion framework (corfu, cape, vertico, orderless, marginalia, consult & embark).
- Email (mu4e & offlineimap)
- IRC (erc & rcirc)
...
- Document manipulation support (PDF/EPUB/HTML/LATEX/MD/ORG, etc) along with viewing aid (olivetti & bionic reading modes).
...
NOTE: For a full list of features please take time to view this configuration in its entirety, the aforementioned list is only some notable features/packages.

** Usage

Please feel free to import snippets from my configuration into your own configuration if you want to or consider forking this repo & maintaining it with your own customizations.

Edit =globals.org= to your liking.

If you would like to use this configuration whilst pulling from upstream, then consider placing all your customizations in =userconfig.org=.
...
NOTE: This configuration is tangled ahead of time for imperative performance reasons and to remove the dependency on org-mode during bootstrap, as a result you will have to use =tangle.sh= in =~/.emacs.d/bin= after you make changes to any =*.org= configuration files.
...
Careful consideration regarding performance optimizations has gone into designing this early-init.

NOTE: Regarding GC; we up the GC at startup and lower it after initialization, we let GCMH handle GC during normal usage along with a helper function that is hooked onto the minibuffer enter/leave state that triggers further GC, all of this should result in unnoticeable GC during runtime.
...
**** Description
Minimum contrast ratio of 4.5:1 against the main background (WCAG AA standard).
**** Code
#+begin_src emacs-lisp
  (use-package ef-themes
	:disabled t
	:straight (:type git :repo "https://git.sr.ht/~protesilaos/ef-themes"))
#+end_src
