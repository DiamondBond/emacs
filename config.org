...
This document along with its auxiliary files, folders & dependencies (specified [[https://github.com/DiamondBond/emacs/blob/master/docs/dependencies.org][here]]) should result in a modern Emacs experience.

** Features

- A strong, performant & stable foundation with optimizations applied.
- Visually appealing without straying too far from the timeless Emacs appearance.
- Iconography used where possible (all-the-icons in dired & kind-icon for completions)
- IDE-esque features (lsp, neotree, evil, completions, navigation, git porcelain etc) for /most/ languages (for a full list of supported languages please view the [[https://github.com/DiamondBond/emacs/blob/master/config.org#languages][Languages section]]).
- Modular completion framework around (corfu, cape, vertico, orderless, marginalia, consult & embark).
- Email that doesn't suck.
- IRC (erc & rcirc)
- News and infotainment aggregation (elfeed/gnus).
- Media playback control (ncmpcpp/mpd & mpv).
- Advanced window management (golden-ratio & e2wm).
- Document manipulation support (PDF/EPUB/HTML/LATEX/MD/ORG, etc) along with viewing aids (olivetti & bionic reading modes).
...
- Complete search framework (deadgrep, deft & avy) for files & folders.
...
If you would like to use this configuration whilst pulling from upstream every once in a while then consider placing all your customizations in =userconfig.org= & add =userconfig.org= to .gitignore.

Please edit =globals.org= to your liking and add it to your =.gitignore= as well.

Make sure you visit this projects [[https://github.com/diamondbond/emacs][Github repo]] for documentation and other information if you are considering using this configuration on your own machine.
...
You can [[https://github.com/yilkalargaw/org-auto-tangle][read more on tangling here]].
...
- [[https://github.com/DiamondBond/emacs/blob/master/modules/mail.org][email]] - Email configuration
...
- [[https://github.com/DiamondBond/emacs/blob/master/modules/archive/laptop.org][laptop]] - Laptop configuration
- [[https://github.com/DiamondBond/emacs/blob/master/modules/archive/desktop.org][desktop]] - Desktop Configuration
...
Set default browser as Firefox when on Windows.
#+begin_src emacs-lisp
...
#+begin_src sh
  emacsclient -c -n -e '(start-to-org-agenda)' &
#+end_src
