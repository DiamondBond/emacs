...
* Introduction
This is an Emacs distribution that aims to enhance the default Emacs experience. It builds upon defaults where possible and bundles that which is considered necessasry as additional [[file:config.org::*Use-package][use-package]] declarations.

This Emacs distribution is compatible with GNU Emacs 27+. In general though, I recommend running this distribution with the latest Emacs HEAD.
