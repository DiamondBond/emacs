...
It is recommend to run this distribution with the latest Emacs [[git.sv.gnu.org/emacs.git][HEAD]] compiled from source with these [[https://github.com/DiamondBond/emacs/blob/master/README.org#configuration][configure flags]], but it should be compatible with GNU Emacs 27+
