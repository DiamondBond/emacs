...
** Initialize =ein=
*** Description
Emacs IPython Notebook (EIN); jupyter client for all languages.
*** Code
#+begin_src emacs-lisp
  ;; (use-package ein
  ;;   :defer t
...
** Initialize =so-long=
*** Description
When you visit a file with very long lines, Emacs will have trouble fontifying everything properly and performance will suffer as a result. We can prevent Emacs from even attempting to deal with such longs lines by enabling the built-in global-so-long-mode (for Emacs versions >= 27).
*** Code
#+begin_src emacs-lisp
  (use-package so-long
	:ensure t)
...
This handles the following languages:
- C/C++
- JS/JSX/TSX/HTML
- Python
*** Code
...
** Python
*** Description
Blacken & PyVenv [Deprecated] OR elpy [Recommended if not using lsp].
*** Code
#+begin_src emacs-lisp
  ;; (use-package pyvenv
  ;;   :defer t)
...
** Web & HTML
*** Description
Web mode. [Deprecated in favor of lsp]
*** Code
#+begin_src emacs-lisp
...
** Javascript
*** Description
JS support. [Deprecated in favor of lsp]
*** Code
#+begin_src emacs-lisp
...
** ReactJS
*** Description
ReactJS mode. [Deprecated in favor of lsp]
*** Code
#+begin_src emacs-lisp
...
** Typescript
*** Description
TSX support. [Deprecated in favor of lsp]
*** Code
#+begin_src emacs-lisp
...
** Prettier
*** Description
Prettier JS support. [Deprecated in favor of lsp]
*** Code
#+begin_src emacs-lisp
...
** Rename Buffer & File
#+begin_src emacs-lisp
  (defun third-party/rename-current-buffer-file ()
...
** Delete Buffer & File
#+begin_src emacs-lisp
  (defun third-party/delete-current-buffer-file ()
...
#+end_src
** Switch to QEMU & Paste + Compile
#+begin_src emacs-lisp
  (defun switch-to-qemu-and-paste ()
	"Switches to QEMU & pastes clipboard."
	(interactive)
	(mark-whole-buffer)
	(call-interactively 'evil-yank)
	(async-shell-command "sh -c ~/bin/stqap.sh"))
#+end_src
** Switch to QEMU & Paste + Run
#+begin_src emacs-lisp
  (defun switch-to-qemu-and-run ()
	"Switches to QEMU & pastes clipboard + runs executable."
	(interactive)
	(mark-whole-buffer)
	(call-interactively 'evil-yank)
	(async-shell-command "sh -c ~/bin/run_bugzilla_exe.sh"))
#+end_src
** Erc handlers
#+begin_src emacs-lisp
  (defun erc-start ()
...
#+begin_src emacs-lisp
  ...
  ;; (load-theme 'leuven t)
#+end_src
** Enable toolbar for Athena Emacs daemons
#+begin_src emacs-lisp
