...
Welcome to my Emacs configuration, this file is an accumulation of 2+ years worth of [[https://projects.csail.mit.edu/gsb/old-archive/gsb-archive/gsb2000-02-11.html][yak shaving]], I hope you acquire some useful knowledge here.

This document along with its auxiliary files, folders & dependencies (available [[https://github.com/DiamondBond/emacs/blob/master/docs/dependencies.org][here]]) should entail a modern, feature-rich & complete Emacs operating system.

** Usage

Please feel free to import snippets from my configuration into your own configuration if you want or consider forking this repo & editing/maintaining it with your own customizations if you so desire (I accept pull requests!).

If you would like to use this configuration whilst pulling from upstream every once in a while then consider placing all your customizations in =userconfig.org= and adding that configuration file to your =.gitignore= within this directory so as to not conflict with upstream changes (although there should rarely ever be any changes to the userconfig.org unless absolutely necessary.)

Please edit =globals.org= to your liking and add it to your =.gitignore= as well.

Make sure you visit this projects [[https://github.com/diamondbond/emacs][Github repo]] if you are considering using this configuration on your own machine.

I would also recommend maintaining your =userconfig.org= elsewhere and linking it to within this repo so you won't ever lose it, also make periodic backups of this file.

NOTE: This configuration is tangled ahead of time for performance reasons and to remove the dependency on org-mode during bootstrap, as a result you will have to use =tangle.sh= in =~/.emacs.d/bin= after you make changes to any =*.org= configuration files.
You can [[https://github.com/diamondbond/emacs#tangling][read more on tangling here]].

** Features

- A strong, performant & stable foundation with all possible optimizations applied.
- Visually appealing without straying too far from the timeless Emacs appearance.
- Icons used where possible (all-the-icons for dired, etc & kind-icon for completions) for improved visual perception and transmission of concepts/information to the user.
- IDE-esque features (lsp, neotree, evil, completions, navigation, etc) for /most/ languages (for a full list of supported languages please view the [[https://github.com/DiamondBond/emacs/blob/master/config.org#languages][Languages section]]) along with the best git porcelain (magit) and forge integration.
- The best completion framework around (corfu, cape, vertico, orderless, marginalia, consult & embark).
- Full Email management capabilities.
- News and infotainment aggregation (elfeed/gnus).
- Media playback control (ncmpcpp/mpd & mpv).
- Window management (e2wm & golden-ratio windows).
- Built in IRC configured.
- Document viewing and editing support (PDF/Epub, etc) along with viewing helpers (olivetti & bionic reading modes).
- Org-mode planning & scheduling as well as featureful notes and exporting them to a variety of formats.
- Fully featured writing capabilities; spellcheck, dictionary & thesaurus lookup, flesch-kincaid reports, quick engine search, common academic phrases help and etc...
- Complete search framework (deadgrep, deft & avy) for files & folders and characters within files.

NOTE: For a full list of features please view the entire configuration, the aforementioned list is only some notable packages/features.
...
- [[https://github.com/DiamondBond/emacs/blob/master/early-init.el][early-init]] - Early initialization

*** *init.el*

- [[https://github.com/DiamondBond/emacs/blob/master/init.el][init]] - Initialize core & straight.el
...
- [[https://github.com/DiamondBond/emacs/blob/master/modules/evil.org][evil]] - Evil configuration
- [[https://github.com/DiamondBond/emacs/blob/master/modules/mail.org][mail]] - Mail configuration
- [[https://github.com/DiamondBond/emacs/blob/master/modules/reddit.org][reddit]] - Reddit configuration
- [[https://github.com/DiamondBond/emacs/blob/master/modules/discord.org][discord]] - Discord configuration
...
Many performance optimizations and considerations have gone into designing this file, please refrain from editing it unless absolutely necessary.

NOTE: Regarding GC; we up the GC at startup and lower it after initialization, we then let GCMH handle GC along with a helper function that is hooked onto the minibuffer enter/leave state that triggers further GC, all of this should result in unnoticeable GC during runtime.
...
This section tangles out to =init.el= and is considered feature complete as all it needs to accomplish is handling the bootstrapping of straight.el and loading of the base & user configuration, along with restoring default GC parameters right after initialization.
...
#+begin_src emacs-lisp
  ...
  (when (file-readable-p
		 (concat user-emacs-directory "globals.el"))
	(load-file
	 (concat user-emacs-directory "globals.el")))
#+end_src
** Personal
...
#+begin_src emacs-lisp
  (set-scroll-bar-mode 'right) ;; Enable right scrollbar for x-toolkit=athena
...
#+begin_src emacs-lisp
  ...
  (global-set-key (kbd "C-<f8>") 'tool-bar-mode)
  (global-set-key (kbd "C-<f9>") 'tab-bar-mode)
...
#+begin_src emacs-lisp
  ...
  ;; enable org-roam when ~/org/roam exists
...
#+begin_src emacs-lisp
  (use-package dashboard
	...
	(defun globals-edit ()
	  "Edit globals configuration file."
	  (interactive)
	  (if (file-exists-p (concat user-emacs-directory "globals.org"))
		  (find-file (concat user-emacs-directory "globals.org"))))
	(defun userconfig-edit ()
	  "Edit user configuration file."
	  (interactive)
	  (if (file-exists-p (concat user-emacs-directory "userconfig.org"))
		  (find-file (concat user-emacs-directory "userconfig.org"))))
	(when (file-directory-p "~/org")
	  (defun notes-edit ()
		"Edit notes file."
		(interactive)
...
#+end_src
** Tools
*** Initialize =org-auto-tangle=
Auto tangle org files on save.
#+begin_src emacs-lisp
...
*** Initialize =e2wm=
**** Description
Equilibrium Emacs Window Manager.
...
#+begin_src emacs-lisp
  (use-package e2wm
	:straight t
	:config
	;; configure e2wm dashboard
	(setq e2wm:c-dashboard-plugins
		  '((open :plugin-args (:command mu4e :buffer "*mail*"))
			(open :plugin-args (:command eshell :buffer "*eshell*"))))
	;; keybinds to start e2wm
...
*** Initialize =forge=
**** Description
Additional git interactions with Forge.
**** Code
#+begin_src emacs-lisp
  (use-package forge
	:straight t)
...
**** Description
Travel back and forth through a files history.
**** Code
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  ...
  (use-package embark-consult
	...
	(embark-collect-mode . consult-preview-at-point-mode))
#+end_src
*** Initialize Emacs Adjustments for Completion
**** Description
...
** Jupyter
*** Description
Jupyter notebook repl & src blocks in org-mode, also provides kernel interactions integrated with Emacs's built-in features.
...
** Config
*** =config/tangle=
Tangles Emacs configuration.
#+begin_src emacs-lisp
  (defun config/tangle ()
	"Tangle this Emacs configuration."
	(interactive)
	(when (file-readable-p
		   (concat user-emacs-directory "bin/tangle.sh"))
	  (shell-command "bin/tangle.sh")))
#+end_src
*** =config/reload=
