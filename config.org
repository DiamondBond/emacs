...
* Preamble
This is an emacs configuration based upon witchmacs
* Dependencies
** Packages
=clang= & =clangd= as backend for C and C++ autocompletion
=llvm= & =libclang-dev= to install irony server
=ripgrep= & =fd=find= for file navigation
=imagemagick= for images
=texlive-latex-extra= for exporting to LaTeX & PDF
=notmuch= , =offlineimap= & =msmtp= for email support
...
#+end_src
** Files
Certain files & folders must exist for correct operation of this emacs config.

*org-mode*
#+begin_quote
~/org/export exists
~/org/inbox.org exists and is readable
~/org/.orgids can be written to
~/Dropbox/org exists
~/org/inbox.org exists
~/org/journal.org exists
#+end_quote
...
~/org exists
~/pdfs exists
~/org/notes.org exists
~/.emacs.d/init.el exists and contains no errors.
~/.emacs.d/config.org exists and contains no errors.
#+end_quote

*notmuch*
#+begin_quote
~/mail
~/.offlineimaprc (included within this repo, make sure to update email and passwd references within this file)
~/.config/offlineimap
~/.config/offlineimap/postsync.sh (included within this repo, must be set with correct permissions; chmod +x postsync.sh)
#+end_quote

*folders*
This emacs configuration also depends on a few folders to exist, most notably:
#+begin_quote
~/.emacs.d		# This houses the emacs configuration (this repo)
~/Dropbox/org	# This is where I sync my working org-mode files to
#+end_quote
** Emacs
...
#+end_src
*** Configuration
We configure emacs to compile with the following:
- json for improved LSP performance.
- nativecomp for overall improved performance.
#+begin_src sh
  ./autogen.sh
