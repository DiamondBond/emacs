...
*** Initialise =mu4e=
**** Description
mu4e is an email client for Emacs. It's based on the mu email indexer/searcher. It's not information overload. It's filter failure. mu4e's mission is to be a better filter.
**** Code
#+begin_src emacs-lisp
