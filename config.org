...
** Normalize paragraph
#+begin_src emacs-lisp
  (defun vz/normalise-paragraph ()
	"`vz/unfill-paragraph' then double spaces -> single space."
	(interactive)
	(let ((end (set-marker (make-marker) (if (use-region-p) (region-end) nil))))
	  (vz/unfill-paragraph)
	  (save-excursion
		(beginning-of-line)
		(while (re-search-forward "\\([]\"')]?\\)\\([.?!]\\)\\([]\"')]?\\)  "
								  (if (marker-position end) end (line-end-position))
								  'return-nil)
		  (replace-match "\\1\\2\\3 ")))
	  (set-marker end nil)))
#+end_src
