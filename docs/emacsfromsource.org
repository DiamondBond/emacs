...
#+begin_src sh
  ...
  # Configure Emacs with Athena toolkit
  ./configure --with-dbus --with-webp --with-gif --with-jpeg --with-png --with-rsvg --with-tiff --with-xft --with-xpm --with-gpm=no --disable-silent-rules --with-modules --with-file-notification=inotify --with-mailutils --with-x=gtk3 --with-x-toolkit=athena --without-gconf --without-gsettings --with-lcms2 --with-imagemagick --with-xml2 --with-json --with-sqlite3 --with-harfbuzz --without-compress-install --with-native-compilation --with-xinput2 --with-tree-sitter CFLAGS="-O3 -mtune=native -march=native -fomit-frame-pointer -flto -fno-semantic-interposition"
