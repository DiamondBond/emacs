...
#+end_src
** Garbage collection in minibuffer
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  ...
  (define-key z-map (kbd "B") 'gnus-group-browse-foreign-server)
...
Org-tree-slider and ox-beamer included for presentation support.
** Code
#+BEGIN_SRC emacs-lisp
  (use-package org
	:ensure t
